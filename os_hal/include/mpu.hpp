 ...
 *
 * 必需静态constexpr：
 * - bool is_available：编译期常量，指示 MPU 功能是否可用
 *
 * 必需静态方法：
...
/**
 * @brief 检测类型 T 是否包含常量 is_available
 * @tparam T 待检测的类型
 */
template <typename T, typename = void>
struct has_is_available_constant : std::false_type {};
template <typename T>
struct has_is_available_constant<T, std::void_t<decltype(T::is_available)>> : std::true_type {};
template <typename T>
static constexpr bool has_is_available_constant_v = has_is_available_constant<T>::value;

/**
 * @brief 检测类型 T 的 is_available 是否为 const bool 类型
 * @tparam T 待检测的类型
 */
template <typename T>
struct is_valid_is_available_constant : std::is_same<decltype(T::is_available), const bool> {};
template <typename T>
static constexpr bool is_valid_is_available_constant_v = is_valid_is_available_constant<T>::value;
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_is_enabled_return : std::false_type {};
template <typename T>
struct is_correct_mpu_is_enabled_return<T, std::void_t<decltype(T::is_enabled())>>
    : std::is_same<decltype(T::is_enabled()), bool> {};
template <typename T>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_is_mpu_fault_return : std::false_type {};
template <typename T>
struct is_correct_mpu_is_mpu_fault_return<T, std::void_t<decltype(T::is_mpu_fault())>>
    : std::is_same<decltype(T::is_mpu_fault()), bool> {};
template <typename T>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_get_fault_address_return : std::false_type {};
template <typename T>
struct is_correct_mpu_get_fault_address_return<T, std::void_t<decltype(T::get_fault_address())>>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_region_count_return : std::false_type {};
template <typename T>
struct is_correct_mpu_region_count_return<T, std::void_t<decltype(T::region_count())>>
    : std::is_same<decltype(T::region_count()), typename T::region_index_type> {};
template <typename T>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_get_region_return : std::false_type {};
template <typename T>
struct is_correct_mpu_get_region_return<
    T,
    std::void_t<decltype(T::get_region(std::declval<typename T::region_index_type>()))>>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_is_readable_return : std::false_type {};
template <typename T>
struct is_correct_mpu_is_readable_return<
    T,
    std::void_t<decltype(T::is_readable(std::declval<typename T::region_address_type>()))>>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_is_writable_return : std::false_type {};
template <typename T>
struct is_correct_mpu_is_writable_return<
    T,
    std::void_t<decltype(T::is_writable(std::declval<typename T::region_address_type>()))>>
 ...
 */
template <typename T, typename = void>
struct is_correct_mpu_is_executable_return : std::false_type {};
template <typename T>
struct is_correct_mpu_is_executable_return<
    T,
    std::void_t<decltype(T::is_executable(std::declval<typename T::region_address_type>()))>>
...
template <typename T>
static constexpr bool is_valid_mpu_policy_v = has_is_available_constant_v<T> && is_valid_is_available_constant_v<T> &&
                                              (T::is_available ? is_valid_mpu_policy<T>::value : true);
} // namespace strat_os::hal::traits
...
/**
 * @brief MPU 适配器模板
 * @tparam MpuPolicy 具体的策略类，对于is_available = true，必须满足 MPU 策略接口，否则隐藏所有接口
 *
...
struct Mpu {
    ...
    /// MPU 可用性常量（取自策略类）
    static constexpr bool is_available = MpuPolicy::is_available;

    // ----- 细粒度静态断言，提供清晰的错误信息 -----
    static_assert(traits::has_is_available_constant_v<Policy>, "Policy must define static constant is_available");
    static_assert(traits::is_valid_is_available_constant_v<Policy>, "is_available must be of type const bool");

    static_assert(!MpuPolicy::is_available || traits::has_region_index_type_v<Policy>, "Policy must define region_index_type");
    static_assert(!MpuPolicy::is_available || traits::has_region_address_type_v<Policy>, "Policy must define region_address_type");
    static_assert(!MpuPolicy::is_available || traits::has_region_size_type_v<Policy>, "Policy must define region_size_type");
    static_assert(!MpuPolicy::is_available || traits::has_region_attributes_type_v<Policy>,
                  "Policy must define region_attributes_type");
    static_assert(!MpuPolicy::is_available || traits::has_region_info_type_v<Policy>, "Policy must define region_info_type");
    static_assert(!MpuPolicy::is_available || traits::has_region_info_base_v<Policy>, "region_info_type must have member 'base'");
    static_assert(!MpuPolicy::is_available || traits::has_region_info_size_v<Policy>, "region_info_type must have member 'size'");
    static_assert(!MpuPolicy::is_available || traits::has_region_info_attr_v<Policy>, "region_info_type must have member 'attr'");
    static_assert(!MpuPolicy::is_available || traits::has_region_info_enabled_v<Policy>,
                  "region_info_type must have member 'enabled'");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_enable_method_v<Policy>,
                  "Policy must provide static method enable()");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_disable_method_v<Policy>,
                  "Policy must provide static method disable()");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_is_enabled_method_v<Policy>,
                  "Policy must provide static method is_enabled()");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_is_enabled_return_v<Policy>, "is_enabled() must return bool");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_set_region_method_v<Policy>,
                  "Policy must provide static method set_region(...)");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_disable_region_method_v<Policy>,
                  "Policy must provide static method disable_region(region_index_type)");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_is_mpu_fault_method_v<Policy>,
                  "Policy must provide static method is_mpu_fault()");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_is_mpu_fault_return_v<Policy>,
                  "is_mpu_fault() must return bool");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_get_fault_address_method_v<Policy>,
                  "Policy must provide static method get_fault_address()");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_get_fault_address_return_v<Policy>,
                  "get_fault_address() must return region_address_type");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_clear_fault_method_v<Policy>,
                  "Policy must provide static method clear_fault()");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_region_count_method_v<Policy>,
                  "Policy must provide static method region_count()");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_region_count_return_v<Policy>,
                  "region_count() must return region_index_type");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_get_region_method_v<Policy>,
                  "Policy must provide static method get_region(region_index_type)");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_get_region_return_v<Policy>,
                  "get_region(region_index_type) must return region_info_type");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_is_readable_method_v<Policy>,
                  "Policy must provide static method is_readable(region_address_type)");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_is_readable_return_v<Policy>,
                  "is_readable(region_address_type) must return bool");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_is_writable_method_v<Policy>,
                  "Policy must provide static method is_writable(region_address_type)");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_is_writable_return_v<Policy>,
                  "is_writable(region_address_type) must return bool");
    static_assert(!MpuPolicy::is_available || traits::has_mpu_is_executable_method_v<Policy>,
                  "Policy must provide static method is_executable(region_address_type)");
    static_assert(!MpuPolicy::is_available || traits::is_correct_mpu_is_executable_return_v<Policy>,
                  "is_executable(region_address_type) must return bool");
    static_assert(!MpuPolicy::is_available || traits::is_valid_region_address_type_v<Policy>,
                  "region_address_type must be an unsigned integer type");
    static_assert(!MpuPolicy::is_available || traits::is_valid_region_size_type_v<Policy>,
                  "region_size_type must be an unsigned integer type");
    static_assert(!MpuPolicy::is_available || traits::is_valid_region_index_type_v<Policy>,
                  "region_index_type must be an unsigned integer type");
    static_assert(!MpuPolicy::is_available || traits::is_valid_region_info_enabled_type_v<Policy>,
    ...
    inline static std::enable_if_t<P::is_available> enable() noexcept {
        Policy::enable();
    }
     ...
     */
    template <typename P = Policy>
    inline static std::enable_if_t<P::is_available> disable() noexcept {
        Policy::disable();
    }
     ...
     */
    template <typename P = Policy>
    inline static std::enable_if_t<P::is_available, bool> is_enabled() noexcept {
     ...
     */
    template <typename P = Policy>
    inline static std::enable_if_t<P::is_available> set_region(region_index_type idx,
     ...
     */
    template <typename P = Policy>
    inline static std::enable_if_t<P::is_available> disable_region(region_index_type idx) noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, bool> is_mpu_fault() noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, region_address_type> get_fault_address() noexcept {
     ...
     */
    template <typename P = Policy>
    inline static std::enable_if_t<P::is_available> clear_fault() noexcept {
        Policy::clear_fault();
    }
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static constexpr std::enable_if_t<P::is_available, region_index_type> region_count() noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, region_info_type> get_region(
        region_index_type idx) noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, bool> is_readable(region_address_type addr) noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, bool> is_writable(region_address_type addr) noexcept {
     ...
     */
    template <typename P = Policy>
    [[nodiscard]] inline static std::enable_if_t<P::is_available, bool> is_executable(
        region_address_type addr) noexcept {
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_get_fault_access_type_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_set_background_region_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename = std::enable_if_t<P::is_available && traits::has_mpu_is_background_region_enabled_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_disable_subregion_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_get_subregion_mask_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_is_overlap_method_v<P>>>
     ...
     */
    template <typename P = Policy,
              typename   = std::enable_if_t<P::is_available && traits::has_mpu_region_priority_by_number_method_v<P>>>
